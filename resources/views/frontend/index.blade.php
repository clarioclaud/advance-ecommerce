...
<div class="body-content outer-top-xs" id="top-banner-and-menu">
            ...
            <div class="tab-pane" id="category{{$cat->id}}">
              <div class="product-slider">
                <div class="owl-carousel home-owl-carousel custom-carousel owl-theme">
                  @php
                    $catwise = App\Models\Product::where('category_id',$cat->id)->orderBy('id','DESC')->get();
                  @endphp
                  @forelse($catwise as $product)
                  <div class="item item-carousel">
                    <div class="products">
                      <div class="product">
                        <div class="product-image">
 ...
 <!-- ============================================== skip_brand_0 PRODUCTS ============================================== -->
        <section class="section featured-product wow fadeInUp">
          <h3 class="section-title">
          @if(session()->get('language')=='hindi')   {{ $skip_brand_0->brand_name_hin }} @else   {{ $skip_brand_0->brand_name_en }} @endif
          </h3>
          <div class="owl-carousel home-owl-carousel custom-carousel owl-theme outer-top-xs">
              ...
              <div class="item">
                <div class="blog-post">
                  <div class="blog-post-image">
                    <div class="image"> <a href="blog.html"><img src="{{ asset($blogpost->blog_image) }}" alt=""></a> </div>
                  </div>
                  <!-- /.blog-post-image -->
                  
                  <div class="blog-post-info text-left">
                    <h3 class="name"><a href="{{ route('blog.details',$blogpost->id) }}">@if(session()->get('language') == 'hindi') {{ $blogpost->blog_title_hin }}  @else {{ $blogpost->blog_title_en }}   @endif</a></h3>
                    <span class="info">{{ Carbon\Carbon::parse($blogpost->created_at)->diffForHumans() }}</span>
                    <p class="text">@if(session()->get('language') == 'hindi') {!! Str::limit($blogpost->blog_details_hin,100) !!}  @else {!! Str::limit($blogpost->blog_details_en,100) !!}   @endif</p>
                    <a href="{{ route('blog.details',$blogpost->id) }}" class="lnk btn btn-primary">Read more</a> </div>
                  <!-- /.blog-post-info --> 
